...
export function BlockEditor({ content, onChange }: { content: JournalContent; onChange: (content: JournalContent) => void }) {
  ...
  return <section className="journal-editor-section"><div className="journal-editor-heading"><div><span>02</span><h2>Story blocks</h2></div><div className="journal-block-add">{(["paragraph", "heading", "quote", "pull_quote", "image", "video_embed", "link"] as const).map((type) => <button className="btn btn-secondary btn-sm" type="button" onClick={() => add(type)} key={type}>+ {type.replace("_", " ")}</button>)}</div></div>
